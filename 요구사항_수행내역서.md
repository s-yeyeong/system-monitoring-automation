...
## 5. 크론(Cron) 스케줄러 기반 자동화 구성 및 환경 변수 전역 예외 트러블슈팅
...
#### ② 비대화형(Non-interactive) 쉘 환경 변수 트러블슈팅 (핵심 분석)
* **발생 문제:** `agent-admin` 유저 크론탭에 등록 후 모니터링 스크립트가 정해진 시간에 작동했으나, 로그 파일(`/var/log/agent-app/monitor.log`)에 아무런 데이터가 적히지 않는 현상 발생.
* **원인 분석:** 리눅스 `cron` 스케줄러는 사용자가 직접 로그인하여 사용하는 대화형 쉘과 달리, 독립적인 '비대화형 쉘(Non-interactive Shell)' 상태로 구동됨. 이로 인해 유저의 개인 설정 파일인 `~/.bashrc`에 선언된 `$AGENT_LOG_DIR` 등의 환경 변수 주소를 인식하지 못해 데이터 적재 경로를 완전히 잃어버린 것으로 판명됨.
* **해결 조치:** 스케줄러가 타 시스템 환경 변수에 의존하지 않고 독립적으로 경로를 역추적할 수 있도록, 관제 스크립트(`monitor.sh`) 내부 코드를 **'절대 경로(Absolute Path) 기반 구조'**로 전면 수정하여 원천적으로 경로 유실을 차단함.

```bash
# 관제 스크립트 내부 구조를 절대 경로 기반 필살기로 개정 및 작성
cat << 'EOF' > /home/agent-admin/agent-app/monitor.sh
#!/bin/bash

# Cron 데몬 환경에서도 길을 잃지 않도록 시스템 절대 경로를 직접 선언
AGENT_LOG_DIR=/var/log/agent-app
LOG_FILE=$AGENT_LOG_DIR/monitor.log
NOW=$(date "+%Y-%m-%d %H:%M:%S")

# 실행 중인 파이썬 프로세스 ID 추출
PID=$(pgrep -f agent_app.py)
...
# 2. 해당 계정 고유의 크론 스케줄 편집기 오픈 (최초 진입 시 1번 에디터 nano 지정)
crontab -e

# 3. 편집기 최하단에 매 분마다 무조건 관제 스크립트를 밀어 넣는 스케줄링 규칙 선언 후 저장
* * * * * /home/agent-admin/agent-app/monitor.sh
```

### 5-3. 수행 결과 및 검증 데이터 (증거 자료)
* 설정이 완비된 직후 사용자가 키보드로 어떤 감시 명령어도 수동 입력하지 않았음에도 불구하고, 타임스탬프 시계가 정확히 1분 간격으로 자동 갱신되며 상태 확인 파일이 성공적으로 기록되는 것을 검증함.
* 서버 완전 재부팅 혹은 일시적인 서비스 오프라인 상태(`[CRITICAL]`) 이후, 백그라운드 수동 복구에 따라 프로세스 번호(`PID`)를 실시간으로 다시 추적 및 캐치하여 정상 상태(`[INFO]`)로 연속 전환 로깅이 이루어지는 자동 관제 프로세스를 최종 확정함.
